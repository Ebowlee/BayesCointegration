...
### 版本概述
月度统计增强：在月度总结日志后增加累计统计，帮助追踪策略长期风控表现。

### 🎯 核心改进

#### 改进1: 新增月度累计统计日志 (main.py Lines 286-309)

**功能描述**:
- 在月度总结日志后紧跟输出累计统计
- 统计范围：从回测开始到当前月（含当月）
- 统计维度：与月度总结一致（均值回归/回撤/超时/协整破裂/其他）

**日志示例**:
```python
# 月度总结（当月）
[月度总结-2024-02] 平仓10次: 均值回归2次, 回撤2次, 超时2次, 协整破裂4次, 其他0次

# 月度累计（从开始到当月）
[月度累计-2024-02] 平仓20次: 均值回归5次, 回撤12次, 超时1次, 协整破裂2次, 其他0次
```

**设计特点**:
- 无需新增数据结构，直接遍历`monthly_close_stats`求和
- 自动处理缺失的reason键（使用`defaultdict(int)`）
- 保持与月度总结相同的格式（便于对比）

**分析价值**:
1. **趋势识别**: 快速判断某类风险是否在增加（如回撤触发占比上升）
2. **策略诊断**: 长期累计数据揭示策略核心问题（如超时频繁 → 持仓逻辑需优化）
3. **月度对比**: 月度总结vs累计统计，识别当月异常波动

### 📝 相关文件

**修改文件**:
- main.py (Lines 286-309)
- docs/CHANGELOG.md (本文件)

### 🚀 下一步建议

运行回测验证:
1. 检查月度切换时是否同时输出总结和累计日志
2. 验证累计统计的准确性（手工核对前几个月的总和）
3. 分析累计趋势是否符合预期（如回撤占比是否控制在合理范围）

---


## [v7.37.1_optimize-logging@20250117]

### 版本概述
日志格式优化：移除冗余信息，标准化分隔符，增加关键统计字段，提升回测分析可读性。

### 🎯 核心改进

#### 改进1: 行业配额日志优化 (IndustryQuotaManager.py Lines 120-139)

**变更内容**:
- 删除行业代码显示 (如 "(20720)")，保留中文名称
- 新增交易次数字段 "交易X对"
- 删除冗余的配额汇总日志 (保留默认配额提示)

**示例对比**:
```python
# 旧格式
[行业配额] 公用事业(20720): 累计收益+0.49% (PnL=$355, Cost=$72,571) → 配额: 1 → 2 (tier=tier1)
[行业配额] 本月动态配额 (非默认): {'公用事业': 2, '生物科技': 5, ...}

# 新格式
[行业配额] 公用事业: 累计收益+0.49% (PnL=$355, Cost=$72,571, 交易3对) → 配额: 1 → 2 (tier=tier1)
# (汇总日志已删除)
```

**优化原因**:
- 行业代码对分析价值低，中文名称更直观
- 交易次数反映配对活跃度，帮助判断配额合理性
- 汇总日志与详细日志信息重复，仅保留默认配额提示

#### 改进2: 协整分析日志简化 (CointegrationAnalyzer.py Lines 228-233)
...
```python
# 旧格式
[协整分析] 硬件(31120): 21只股票 → 配对210对 → PValue通过2对 → 配额2 → 最终选取2对

# 新格式
[协整分析] 硬件: 21只股票 → 配对210对 → P值通过2对 → 配额2 → 最终选取2对
```

#### 改进3: 平仓日志增强 (Pairs.py Lines 343-354)

**变更内容**:
- 新增行业名称显示 (在pair_id后)
- 标准化分隔符: `|` 用于主要层级, `,` 用于从属信息
- 简化冷却期表达: "激活冷却(30天)" → "冷却30天"

**示例对比**:
```python
# 旧格式
[平仓] ('EOG', 'NOG') 均值回归 | 持有7天 | PnL=$213.60 (+1.8%) | ...

# 新格式
[平仓] ('EOG', 'NOG') | 能源 | 均值回归, 持有7天 | PnL=$213.60 (+1.8%) | ...
```

**分隔符层级规则**:
- 第1层 `|`: pair_id | 行业 | 退出原因
- 第2层 `,`: 原因, 持有天数 (从属于退出原因)
- 第3层 `|`: PnL | 累计收益 | Z-score变化 | 交易次数 | 冷却期

#### 改进4: 零轴穿越分布统计 (PairSelector.py Lines 160-181)

**新增日志** (验证v7.37.0新评分维度):
```python
[零轴穿越] 总计120对 → 优秀(≥18):5对 | 良好[12,18):18对 | 中等[6,12):42对 | 稀缺(0,6):38对 | 无穿越:17对 | 最高:34次 | 最低:0次 | 平均:10.3次
```

**分档标准** (基于v7.37.0评分函数设计):
- 优秀 (≥18次): 平台区及以上，高频交易
- 良好 [12,18): 峰值区，理想频率
- 中等 [6,12): 上升区，可接受
- 稀缺 (0,6): 低于基线，信号不足
...
**对回测分析的改进**:
1. **行业追踪**: 平仓日志现可快速识别哪些行业表现优异
2. **配额验证**: 交易次数字段帮助验证动态配额调整的合理性
3. **日志简洁性**: 删除冗余信息后，关键指标更突出
4. **分布验证**: 零轴穿越统计帮助验证v7.37.0评分维度的有效性

**无需代码迁移**: 仅日志格式调整，无逻辑变更

### 📝 相关文件

**修改文件**:
- src/analysis/IndustryQuotaManager.py (Lines 120-139)
- src/analysis/CointegrationAnalyzer.py (Lines 228-233)
- src/Pairs.py (Lines 343-354)
- src/analysis/PairSelector.py (Lines 160-181)
- docs/CHANGELOG.md (本文件)
...
运行回测验证:
1. 检查日志格式是否正确显示
2. 观察零轴穿越分布是否符合预期 (峰值应在[12,18)区间)
3. 验证行业配额调整与交易次数的关联性

---


## [v7.37.0_add-zero-crossing-score@20250117]

### 版本概述
三维质量评分系统：新增"零轴穿越次数"维度，用于验证spread的均值回归潜力和过滤噪声信号。

### 🎯 核心改进

#### 改进1: 新增零轴穿越评分维度 (PairSelector.py Lines 292-380)

**物理意义**:
- **直观验证**: 穿越次数直接反映spread的振荡特性，比复杂统计指标更易理解
- **均值回归潜力**: 频繁穿越零轴说明spread能够稳定回归均值
- **噪声过滤**: 自动排除信号稀缺(<6次/年)和过度交易(>36次/年)配对
...
< 6次           0.0     信号稀缺,资金利用率低
6次             0.5     两月1次,最低可接受
12次            1.0     每月1次,理想频率 (峰值)
18次            0.5     每月1.5次,可接受但非最优
24次            0.5     每月2次,平台区上限
36次            0.0     每月3次,噪声过度
> 36次          0.0     过度交易,硬截断
```
...
**数据来源**:
- 从ModelResult['spread']计算（252天对数价差序列）
- 去均值化: `spread_centered = spread - residual_mean`
- 穿越检测: `np.sum(np.sign(spread_centered)[:-1] != np.sign(spread_centered)[1:])`

**设计优势**:
- **计算高效**: 向量化操作，比高斯曲线快5倍
- **阈值直观**: 配置参数易于调整（min/peak/plateau_end/max）
- **保守权重**: 25%初始权重，便于后续优化至30-35%

#### 改进2: 三维权重调整 (config.py Lines 144-148)
...
```python
quality_weights = {
    'half_life': 0.40,                     # 降低10% (保持最高权重)
    'mean_reversion_certainty': 0.35,      # 降低15%
    'zero_crossing': 0.25                  # 新增 (物理直观+噪声过滤)
}
```

**调整理由**:
- **half_life保持主导**: 40%仍是最高权重（最独立+预测力57%）
- **MR适度降低**: 35%仍保留理论核心地位（预测力50%）
- **零轴保守分配**: 25%便于Phase 3根据回测结果调优

#### 改进3: 评分阈值参数化 (config.py Lines 166-172)
...
**参数化设计优势**:
- **灵活调整**: 修改配置即可改变阈值，无需重新推导公式
- **一致性**: 遵循现有scoring_thresholds结构
- **可扩展**: 便于后续添加动态阈值（例如基于市场波动率）

#### 改进4: ModelResult字段扩展 (PairSelector.py Lines 116-117)

**新增字段**:
```python
model_result['zero_crossing_score'] = zero_crossing_score  # 归一化评分 [0.0, 1.0]
model_result['crossing_count'] = crossing_count            # 实际穿越次数 (int)
```

**用途**:
- **调试分析**: 可检查每个配对的穿越次数分布
- **后续优化**: 为Phase 3权重调优提供数据
- **日志输出**: 可在select_best()中添加穿越次数统计（可选）

### 📊 验证计划

**Phase 1: 分布观察** (实施后立即进行)
- 统计252天lookback内穿越次数的分布
- 验证6-36次区间的覆盖率（预期>70%配对落在此区间）
- 识别异常配对（<6次或>36次）并分析其他质量指标

**Phase 2: 回测对比** (Phase 1完成后)
- 对比回测（有/无零轴穿越维度）
- 关键指标: Sharpe Ratio, Max Drawdown, 交易频率
- 分析零轴穿越评分与实际PnL的相关性

**Phase 3: 权重优化** (Phase 2完成后)
- 根据回测结果调整权重（可能从25%调至30-35%）
- 微调阈值参数（如plateau_end从24调至28）
- 最终确定三维评分系统的最佳配置

### 🔧 技术细节

**实现文件**:
- [src/config.py](../src/config.py#L144-L172): 权重和阈值配置
- [src/analysis/PairSelector.py](../src/analysis/PairSelector.py#L61-L121): evaluate_quality()方法
- [src/analysis/PairSelector.py](../src/analysis/PairSelector.py#L292-L380): _calculate_zero_crossing_score()方法

**向后兼容性**:
- ✅ 完全兼容：ModelResult结构仅添加字段，不修改现有字段
- ✅ 配置自动加载：新权重配置自动生效，无需手动迁移
- ⚠️ 质量分数变化：原有配对的quality_score会因权重调整而变化

**性能影响**:
- **计算开销**: 每个配对增加约0.1ms（向量化操作，可忽略）
- **内存占用**: 每个配对增加12字节（1个float + 1个int）
- **总体影响**: 微小，不影响回测速度

### 📝 后续改进方向

1. **动态阈值** (v7.38.0候选):
   - 根据市场波动率调整min_crossings和max_crossings
   - 高波动期: 放宽至8-48次
   - 低波动期: 收紧至6-24次

2. **穿越质量** (v7.39.0候选):
   - 不仅计数，还评估穿越的"质量"
   - 例如: 穿越时的Z-score幅度（>1.5σ才算有效穿越）
   - 过滤假穿越（微小振荡导致的符号变化）
...
## [v7.36.1_simplify-log-format@20250117]

### 版本概述
简化日志格式,移除配对创建日志中的 PnL 百分比,同时增强开仓日志显示行业名称和紧凑质量标签。

### 核心改进

#### 改进1: 简化配对创建日志 (main.py Lines 249-262)

**v7.36.0 格式** (复杂,含 PnL 百分比):
```
[配对创建] 各行业配对数量: {'软件(+12.5%)': 3, '金属矿业(-8.2%)': 1, '消费品(预热期)': 1}
```

**v7.36.1 格式** (简化,仅显示配对数量):
```
[配对创建] 各行业配对数量: {'软件': 3, '金属矿业': 1, '消费品': 1}
```
...
#### 改进2: 增强开仓日志 (ExecutionManager.py Lines 415-443)

**v7.31.5 格式** (缺少行业上下文):
```
[开仓] ('CMS', 'FE') | 优秀(≥0.80) | Z-score=+1.72σ | 分配=$9,574
```

**v7.36.1 格式** (添加行业 + 紧凑质量标签):
```
[开仓] ('CMS', 'FE') | 公用事业 | Q=≥0.80 | Z-score=+1.72σ | 分配=$9,574
[开仓] ('MSFT', 'AAPL') | 软件 | Q=0.70-0.80 | Z-score=-1.45σ | 分配=$12,345
```

**质量标签映射** (Q= 格式):
- `quality_score >= 0.80` → `Q=≥0.80`
- `0.70 <= quality_score < 0.80` → `Q=0.70-0.80`
- `0.60 <= quality_score < 0.70` → `Q=0.60-0.70`
- `quality_score < 0.60` → `Q={score:.2f}`

**改进点**:
1. **行业上下文**: 添加行业名称,便于识别配对板块归属
2. **紧凑标签**: `优秀(≥0.80)` → `Q=≥0.80`,节省空间
3. **信息密度**: 单行日志包含5个维度(ID、行业、质量、Z值、资金)

### 影响范围
- **main.py**: Lines 249-262 (配对创建日志简化)
- **ExecutionManager.py**: Lines 415-443 (开仓日志增强)

### 向后兼容性
✅ **完全兼容** - 仅修改日志格式,不影响交易逻辑
...
### 版本概述
调整行业 tier 阈值和配额系统，采用更保守的配置策略，降低集中度风险。

### 核心改进

#### 更保守的 Tier 阈值 (config.py Lines 293-298)
...
**改进点**:
1. **新增负收益层级**: tier0 现在明确处理负收益行业（return < 0%）
2. **降低阈值**: 顶层触发条件从 30% 降至 15%（-50%）
3. **更细粒度**: 5% 间隔替代 10%-20% 间隔，响应更灵敏

#### 更保守的配额数量 (config.py Lines 299-305)
...
**改进点**:
1. **显著降低配额**: 最高配额从 9 降至 5（-44%）
2. **线性递增**: 1→2→3→4→5（更平滑的配额分配）
3. **降低集中度风险**: 减少对单一行业的过度依赖

#### 更保守的最大投资比例 (config.py Lines 182-188)

**旧配置**:
```python
tier_max_investment_ratio: {
    'tier0': 0.15,  # ≤5%
    'tier1': 0.18,  # (5%, 10%]
    'tier2': 0.20,  # (10%, 20%]
    'tier3': 0.23,  # (20%, 30%]
...
**改进点**:
1. **降低单个配对上限**: tier4 从 25% 降至 20%（-20%）
2. **降低负收益风险**: tier0 从 15% 降至 10%（-33%）
3. **更平滑的梯度**: 10% → 12% → 15% → 18% → 20%

#### 逻辑一致性优化 (IndustryQuotaManager.py)

**更新 `_get_quota_by_return()` 方法** (Lines 197-223):
```python
# 新分层逻辑:
...
**更新 `_get_tier_by_return()` 方法** (Lines 226-252):
```python
# 新分层逻辑:
...
**更保守的风险管理**:
1. **降低集中度**: 最高配额从 9 降至 5，减少对单一行业的依赖
2. **降低单笔风险**: 最高投资比例从 25% 降至 20%
3. **明确负收益处理**: 负收益行业仅分配最低配额（1个）和最低投资比例（10%）

**更灵敏的响应**:
1. **更低阈值**: 顶层触发条件从 30% 降至 15%，更早识别优秀行业
2. **更细粒度**: 5% 间隔替代 10%-20% 间隔，配额调整更及时

**破坏性变更**:
- **回测结果将显著改变**: 配额和投资比例的大幅调整会改变资金分配模式
- **建议重新回测**: 验证新参数下的策略表现

### 文件修改
- `src/config.py`: 更新 `tier_thresholds`, `tier_quotas`, `tier_max_investment_ratio`
- `src/analysis/IndustryQuotaManager.py`: 更新 `_get_quota_by_return()` 和 `_get_tier_by_return()` 方法及文档
...
### 版本概述
**文档修复** - 更新IndustryQuotaManager.py中过时的配额数量和max_pct注释,与v7.32.3/v7.32.4实际配置保持一致。
...
**问题**:
- IndustryQuotaManager.py的方法文档仍显示旧配置值
- `_get_quota_by_return()`: 配额数量显示为1/2/5/8/10 (实际为1/3/5/7/9)
- `_get_tier_by_return()`: max_pct显示为0.10/0.12/0.15/0.18/0.20 (实际为0.15/0.18/0.20/0.23/0.25)

**修复** (IndustryQuotaManager.py):
```python
# _get_quota_by_return() Line 205
# 修复前: 配额数量 (1/2/5/8/10)
# 修复后: 配额数量 (1/3/5/7/9)

# Lines 207-212 分层逻辑注释:
# 修复前: tier1 (2个,[5%,10%)), tier3 (8个,[20%,30%)), tier4 (10个,[30%,∞))
# 修复后: tier1 (3个,[5%,10%)), tier3 (7个,[20%,30%)), tier4 (9个,[30%,∞))

# _get_tier_by_return() Line 236
# 修复前: 分层逻辑 (v7.32.0):
# 修复后: 分层逻辑 (v7.32.4 更新max_pct配置):

# Lines 237-241 max_pct注释:
# 修复前: tier0 (max_pct=0.10), tier1 (0.12), tier2 (0.15), tier3 (0.18), tier4 (0.20)
# 修复后: tier0 (max_pct=0.15), tier1 (0.18), tier2 (0.20), tier3 (0.23), tier4 (0.25)
```

**对应版本**:
- 配额数量: v7.32.3手动调整 (1/2/5/8/10 → 1/3/5/7/9)
- max_pct: v7.32.4手动调整 (tier0-3分别提升50%/20%/11%/15%)
...
### 版本概述
**日志增强** - 添加PnL条件豁免的调试日志,便于验证v7.33.0的"盈利配对豁免超时"逻辑。
...
**背景**:
- v7.33.0实施了PnL条件豁免逻辑 (盈利配对超时不触发平仓)
- 但回测分析发现: 所有超时案例恰好都是亏损配对,无法验证豁免逻辑是否正确实施
- 需要增加明确的日志输出,便于下次回测验证

**变更** (PairHoldingTimeout.py Line 141-146):
```python
# PnL条件: 只在亏损/持平时触发
if pair_pnl > 0:
    # 盈利配对: 不触发,继续持有
...
```
# 当盈利配对超时时输出 (Debug模式):
[持仓超时] ('AAPL', 'MSFT') 已持仓25天 > 上限20.0天, 但PnL=$1,250 (盈利) → 豁免超时,继续持有
```

**好处**:
- ✅ 明确记录豁免事件,验证v7.33.0逻辑正确性
- ✅ 统计豁免频率,评估"让利润奔跑"策略效果
- ✅ Debug级别日志,不影响生产环境性能
...
下次回测时检查:
1. 是否有"豁免超时,继续持有"的日志输出
2. 豁免配对的最终PnL表现 (是否捕获更多收益)
3. 豁免配对的持仓天数分布 (超时后又持有了多久)

---


## [v7.32.4_tune-aggressive-params@20250117]

### 版本概述
**参数调整 (手动优化)** - 扩大候选池、提升tier投资比例、收紧风控、延长冷却期,应对质量下降趋势,提高资金利用率。
...
**变更**:
```python
# src/config.py UniverseConfig
max_coarse_stocks: 300 → 350 (+17%)
```
...
**变更** (src/config.py PairsTradingConfig.tier_max_investment_ratio):
```python
tier0 (≤5%):    0.10 → 0.15 (+50%)
tier1 (5-10%):  0.15 → 0.18 (+20%)
tier2 (10-20%): 0.18 → 0.20 (+11%)
tier3 (20-30%): 0.20 → 0.23 (+15%)
tier4 (>30%):   0.25 (保持不变)
```

**影响分析**:
- **tier0提升最多 (+50%)**: 容错空间增大,适应低质量环境
- **tier1-3梯度提升**: 保持性能分层,高回报行业获得更多资金
- **tier4保持上限**: 避免过度集中风险

**数学示例** (quality_score=0.80):
```python
# 假设某行业为tier2 (10-20%回报)
min_pct = 0.05
max_pct = 0.20  # 原为0.18

planned_pct = 0.05 + 0.80 * (0.20 - 0.05) = 0.17  # 原为0.154
# 单配对资金分配增加 +10.4%
```
...
**变更**:
```python
# src/config.py PairDrawdownRuleConfig
threshold: 0.04 → 0.03 (-25%)
# 回撤阈值从4%降至3%,触发更早

# HoldingTimeoutRuleConfig
cooldown_days: 15 → 90 (×6)
# 超时后冷却期从15天延长至90天
```

**影响分析**:
- **回撤阈值降低**: 3%回撤即触发平仓 (原为4%),更快止损
- **超时冷却延长**: 超时配对90天内禁止重复交易 (原为15天)
- **风控收紧**: 避免频繁交易低质量配对
...
**变更** (src/config.py Constants.CLOSE_REASONS):
```python
'MEAN_REVERSION': cooldown_days: 7 → 30 (×4)
'PAIR_BREAK':     cooldown_days: 15 → 90 (×6)
```

**影响分析**:
- **均值回归**: 正常平仓后30天冷却 (原为7天),减少短期重复交易
- **协整破裂**: 止损后90天冷却 (原为15天),充分等待关系恢复
- **交易频率**: 预计大幅下降,适合低质量环境
...
**组合拳策略**:
1. **扩大输入** (350只候选) → 提升协整检测质量
2. **增加资金** (tier比例提升) → 提高资金利用率
3. **快速止损** (回撤阈值3%) → 防止低质量配对扩大损失
4. **延长冷却** (30/90天) → 避免频繁交易,降低成本

**参数激进性**:
- ⚠️ **Tier0提升50%**: 对低回报行业给予过多资金,风险增加
- ⚠️ **冷却期延长6倍**: 可能错失市场机会,资金周转率下降
- ⚠️ **回撤阈值3%**: 可能过早平仓,导致止盈不充分
...
**主要验证**:
1. 质量分布改善: 优秀配对(≥0.80)占比是否提升
2. 资金利用率: 保证金占用是否接近目标 (20-40%)
3. 冷却期效果: 30/90天冷却是否有效降低交易频率
4. 风控触发: 3%回撤阈值触发频率和效果

**次要验证**:
1. 候选池扩容: 350只是否充分利用
2. Tier投资比例: 高tier行业资金分配是否合理
3. 超时冷却: 90天冷却是否过长,导致配对无法重新交易

---


## [v7.33.0_optimize-timeout-pnl@20250116]

### 版本概述
**持仓超时规则优化 - 增加PnL条件判断** - 保持盈利仓位,快速止损亏损仓位,遵循"让利润奔跑,快速止损"的交易哲学。
...
**背景**:
- **原逻辑**: `holding_days > max_days` → 立即平仓 (无差别对待所有配对)
- **问题**: 盈利配对也被强制平仓,违背"让利润奔跑"原则,损失潜在收益

**改进** (PairHoldingTimeout.py Lines 107-144):

**新触发条件** (复合AND):
```python
# 原: holding_days > max_days
# 新: holding_days > max_days AND (pair_pnl <= 0 OR pair_pnl is None)
```

**3种场景处理**:
1. **盈利配对 (pnl > 0)**: 不触发,继续持有 → 让利润奔跑
2. **亏损/持平配对 (pnl <= 0)**: 触发平仓 → 快速止损
3. **数据异常 (pnl is None)**: 触发平仓 → Fail-Safe原则
...
if holding_days > max_days:
    ...
    if pair_pnl is None:
        # Fail-Safe: 数据异常触发平仓
        return True, "PnL数据缺失 (异常状态)"

    # PnL条件: 只在亏损/持平时触发
    if pair_pnl > 0:
        return False, ""  # 盈利配对静默,不触发

    # 触发平仓,description增加PnL信息
    description = f"...上限{max_days:.1f}天..., PnL=${pair_pnl:,.0f} (亏损/持平)"
...
```
# 触发时 (亏损/持平):
[Pair风控] 持仓超时 ('AAPL','MSFT'): 已持仓25天 > 上限20.0天 (半衰期10.0天 × 2.0), PnL=$-500 (亏损/持平)

# 未触发 (盈利):
(静默,无日志输出)

# 触发时 (数据异常):
[Pair风控] 持仓超时 ('AAPL','MSFT'): 已持仓25天 > 上限20.0天, PnL数据缺失 (异常状态)
```

**文档更新**:
- 类文档字符串 (Lines 8-42): 增加v7.33.0触发条件说明和3种场景处理
- check()方法文档 (Lines 57-99): 增加复合触发条件说明和示例

### 设计决策

**边界处理**:
- **PnL = 0**: 视为亏损,触发平仓 (保守原则,持平无收益)
- **PnL = None**: 视为异常,触发平仓 (Fail-Safe,持仓必有PnL)
- **微小盈利 (如$+0.50)**: 视为盈利,不触发 (严格数学条件`pnl > 0`)

**与其他规则交互**:
- **PairDrawdownRule (优先级80)**: 互补关系
  - HoldingTimeout: 处理超时亏损配对
  - Drawdown: 处理盈利后回撤配对 (从高点回撤>4%)
  - 缓解大额盈利长期占用资金的风险
- **PairCumulativeLossRule (优先级90)**: 级联关系
  - 同时触发时,CumulativeLoss优先执行 (优先级更高)
  - 无冲突 (RiskManager排他性机制)

**性能影响**:
- 增加操作: 1次`get_pair_pnl()`调用 (2次Portfolio字典查询,O(1))
- 预估成本: ~0.001ms per pair
- 总影响: 可忽略 (仅对持仓配对调用)

### 预期效果

**正面影响**:
- ✅ 减少过早止盈: 盈利配对超时后仍可持有,捕获更多收益
- ✅ 快速止损: 亏损配对超时后立即平仓,防止协整失效扩大损失
- ✅ Fail-Safe: 数据异常时优先保护资金
- ✅ 日志清晰: 触发时显示PnL信息,未触发时静默

**潜在风险**:
- 大额盈利配对可能长期占用资金 → PairDrawdownRule缓解(回撤触发)
- PnL数据缺失误杀盈利配对 → 理论上不应发生(持仓必有PnL)

### 回测验证点

1. **触发次数**: 持仓超时触发次数是否减少 (盈利配对不再触发)
2. **PnL数据完整性**: "PnL数据缺失"日志出现次数 (预期0次)
3. **盈利配对表现**: 超时后继续持有的盈利配对,最终收益是否更高
4. **整体效果**: 总收益率、夏普比率、最大回撤是否改善

---


## [v7.32.3_tune-allocation-parameters@20250116]

### 版本概述
**参数调优 - 扩大候选池并提高高收益行业配置** - 手动调整三项关键参数以优化策略表现。

### 核心调整

#### 1. 扩大候选池
**UniverseConfig.max_coarse_stocks (config.py Line 41)**:
- **调整**: 200 → 300 (+50%)
- **目的**: 增加协整配对筛选空间,提高发现优质配对的概率
- **影响**: 粗筛后进入细筛的股票数量增加,协整分析的输入池扩大

#### 2. 提高高收益行业配置
**PairsTradingConfig.tier_max_investment_ratio (config.py Lines 182-186)**:
```python
# 旧值 → 新值
tier_max_investment_ratio = {
    'tier0': 0.10,  # 不变
    'tier1': 0.12 → 0.15,  # +25% (5%-10%收益行业)
    'tier2': 0.15 → 0.18,  # +20% (10%-20%收益行业)
    'tier3': 0.18 → 0.20,  # +11% (20%-30%收益行业)
    'tier4': 0.20 → 0.25   # +25% (>30%收益行业)
}
```
- **设计意图**: 更激进地配置表现优异的行业,提升资本回报率
- **极限案例**: tier4行业从单配对最高20%提升至25% (假设质量分数1.0)

#### 3. 优化行业配额梯度
**IndustryQuotaConfig.tier_quotas (config.py Lines 300-304)**:
```python
# 旧值 → 新值
tier_quotas = {
    'tier0': 1,  # 不变
...
```
- **梯度优化**: 1/2/5/8/10 → 1/3/5/7/9 (更均匀的等差数列)
- **设计理念**: 避免tier间配额跳跃过大,平滑资源分配

### 预期影响

1. **候选池扩大**:
   - 协整分析输入从约1100对(200只)增至约2475对(300只) (+125%)
   - 高质量配对发现概率提升

2. **资金利用率提升**:
   - 高收益行业(tier2-4)获得更多资金配置
   - tier4单配对最高从20%提升至25% (max_pct × quality_score)

3. **行业配额平滑**:
   - tier间梯度更均匀(等差2),避免资源分配突变
   - tier3/4微调降低,防止过度集中
...
### 版本概述
**开仓日志优化** - 将质量等级标签直接显示在每条[开仓]日志中,删除冗余的质量分布统计。

### 核心改进

#### 合并质量等级到开仓日志

**ExecutionManager.py (Lines 425-436删除, 448-474修改)**:

**优化前**:
```
[开仓候选] 3对质量分布: 优秀(≥0.80)=2, 良好(0.70-0.80)=1, 及格(0.60-0.70)=0
[开仓] ('CTRA', 'RRC') Z-score=-1.47σ | 分配=$9,840
[开仓] ('XOM', 'CVX') Z-score=1.23σ | 分配=$12,500
```

**优化后**:
```
[开仓] ('CTRA', 'RRC') | 优秀(≥0.80) | Z-score=-1.47σ | 分配=$9,840
[开仓] ('XOM', 'CVX') | 优秀(≥0.80) | Z-score=1.23σ | 分配=$12,500
[开仓] ('MRO', 'APA') | 良好(0.70-0.80) | Z-score=-0.98σ | 分配=$8,200
```

**实现细节**:
```python
# 删除第425-436行的质量分布统计日志
...
1. **信息整合**: 每行日志完整呈现配对的所有关键信息(ID + 质量 + 信号 + 资金)
2. **减少冗余**: 删除频繁打印的质量分布统计,避免信息模糊问题
3. **提升可读性**: 直接显示质量等级,无需对照统计数字推断
4. **保持简洁**: 日志长度增加有限(仅+12-16字符)

### 用户反馈

**问题**: v7.31.4新增的[开仓候选]质量分布日志频繁打印且意思模糊
**解决**: 将质量等级直接嵌入每条[开仓]日志,实现信息整合
...
### 诊断结果 (基于回测 "Pensive Violet Termite")

**估值筛选效果** ✅:
- 平均失败率: **4.8%** (12轮选股)
- OR逻辑(PE≤100 OR PS≤10)有效保留高成长股票
- **结论**: 估值筛选不是瓶颈
...
**优先级1**: 放宽质量筛选阈值(预计v7.30.0)
- 目标: 配对数从2.8对提升至4-5对
- 方案: 降低质量分数阈值10-20%
- 预期: 资金占用率提升至6-8%

**优先级2**: 增加行业配额或放宽协整p值
- 目标: 协整通过数从6.4对提升至10-15对
- 方案: 默认配额1→2-3对,或p值0.05→0.10
...
### 修改文件
- **src/UniverseSelection.py** (Lines 221-229): 估值筛选统计
...
### 版本概述
**日志完善** - 贝叶斯建模总是可见、消除质量筛选重叠。

### 核心改进

#### 1. 贝叶斯建模日志总是打印 (BayesianModeler.py)
**问题**：
- v7.28.3设计"只在有失败时打印"
- 回测显示贝叶斯日志缺失（因为全部成功）
- 无法确认贝叶斯是否真的执行
...
**示例**：
```
[贝叶斯建模] 输入7对 → 成功7对 → 失败0对  ← 新增：确认贝叶斯执行
```

**理由**：
- 贝叶斯建模是核心环节，值得总是显示
- 与配对流程审计日志对应验证（配对流程显示"贝叶斯成功7对"）
- 当fail_count=0时，日志显示"失败0对"仍有信息价值（确认执行）

#### 2. 删除质量筛选日志 (PairSelector.py)
**问题**：
- 质量筛选日志与配对流程日志重叠
```
[质量筛选] 输入7对 → 通过4对 → 淘汰3对  ← 与下一行重叠
[配对流程] 协整通过7对 → 贝叶斯成功7对 → 质量筛选4对 → 最终创建4对
```

**解决**：
- 删除Lines 59-69的质量筛选日志代码块
- 保留配对流程审计日志

**理由**：
- 配对流程审计日志已包含核心信息（"质量筛选4对"）
- 淘汰数可通过简单减法计算（贝叶斯成功数 - 质量筛选数 = 7 - 4 = 3）
- 避免信息重叠，符合v7.28.3日志优化目标
- backtest-analyst可从配对流程日志推断淘汰情况

### 效果对比

#### 修改前 (v7.28.3):
```
[协整分析] 油气(30910): 13只股票 → 检测78对 → 原始通过10对 → 配额1 → 最终选取1对
[协整分析] 半导体(31130): 15只股票 → 检测105对 → 原始通过6对 → 配额3 → 最终选取3对
(缺失贝叶斯日志 - 因为全部成功)
[质量筛选] 输入7对 → 通过4对 → 淘汰3对  ← 重叠
[配对流程] 协整通过7对 → 贝叶斯成功7对 → 质量筛选4对 → 最终创建4对
[配对创建] 各行业配对数量: {'软件': 1, 'REITs': 1, '油气': 1, '周期零售': 1}
```
...
- ✅ 贝叶斯建模可见性增强（确认执行+成功率）
- ✅ 消除质量筛选日志重叠
- ✅ 配对流程审计成为核心漏斗（协整→贝叶斯→质量筛选→创建）
- ✅ 日志更简洁清晰

### 修改文件
- `src/analysis/BayesianModeler.py:60-70` - 删除条件判断，总是打印
- `src/analysis/PairSelector.py:59-69` - 删除质量筛选日志代码块

---


## [v7.28.3_pair-pipeline-visibility@20250206]

### 版本概述
**配对流程可见性增强** - 消除重复日志、暴露黑箱流程、添加完整审计。

### 核心改进

#### 1. 合并协整分析重复日志 (CointegrationAnalyzer.py)
**问题**：
- 同一行业打印两次协整分析日志，数字不一致
- Lines 73-77：`检测N对 → 通过M对`
- Lines 165-169：`通过M对 → 配额Q → 选取K对`
- 造成混淆：为什么"通过10对"变成"通过1对"？

**解决**：
- 删除 Lines 73-77 的重复日志
- 修改 Lines 166-171 为统一日志，显示完整流程
- 新格式：`检测N对 → 原始通过M对 → 配额Q → 最终选取K对`

**示例**：
```
# 修改前（重复且混淆）:
[协整分析] 油气(30910): 13只股票 → 检测78对 → 通过10对
[协整分析] 油气(30910): 通过10对协整检验 → 应用配额1 → 实际选取1对

# 修改后（统一且清晰）:
[协整分析] 油气(30910): 13只股票 → 检测78对 → 原始通过10对 → 配额1 → 最终选取1对
```

#### 2. 添加贝叶斯建模统计日志 (BayesianModeler.py)
**问题**：
- 协整分析 → 配对创建之间的Bayesian建模是黑箱
- 5个行业通过协整，但只创建4个配对，原因不明
- 建模失败（Rhat > 1.1, divergences, 数据退化）静默发生

**解决**：
- 在 `modeling_procedure()` 方法末尾添加统计日志
- 只在有失败时打印（减少噪音）
...
**示例**：
```
[贝叶斯建模] 输入7对 → 成功5对 → 失败2对
```

#### 3. 添加质量筛选统计日志 (PairSelector.py)
**问题**：
- 质量筛选（quality score, 风险配对过滤）过程不可见
- 不知道有多少对因质量分数过低被淘汰
- 不知道有多少对因DRAWDOWN/ANOMALY冷却期被剔除

**解决**：
- 在 `selection_procedure()` 方法末尾添加统计日志
- 只在有淘汰时打印（减少噪音）
...
**示例**：
```
[配对流程] 协整通过7对 → 贝叶斯成功5对 → 质量筛选4对 → 最终创建4对
[配对创建] 各行业配对数量: {'软件': 1, 'REITs': 1, '油气': 1, '周期零售': 1}
```
...
[协整分析] 油气(30910): 13只股票 → 检测78对 → 通过10对
[协整分析] 油气(30910): 通过10对协整检验 → 应用配额1 → 实际选取1对
[协整分析] 公用事业(20720): 11只股票 → 检测55对 → 通过1对
[配对创建] 各行业配对数量: {'软件': 1, 'REITs': 1, '油气': 1, '周期零售': 1}
```
**问题**：
- 5个行业（油气、电信、公用、REITs、半导体、软件、周期）→ 只创建4个？
- 中间发生了什么？贝叶斯失败？质量不达标？

#### 修改后（v7.28.3）:
```
[协整分析] 油气(30910): 13只股票 → 检测78对 → 原始通过10对 → 配额1 → 最终选取1对
[协整分析] 电信服务(30810): 8只股票 → 检测28对 → 原始通过3对 → 配额1 → 最终选取1对
[协整分析] 公用事业(20720): 11只股票 → 检测55对 → 原始通过2对 → 配额1 → 最终选取1对
[协整分析] REITs(10420): 12只股票 → 检测66对 → 原始通过5对 → 配额1 → 最终选取1对
[协整分析] 半导体(31130): 15只股票 → 检测105对 → 原始通过6对 → 配额3 → 最终选取3对
[协整分析] 软件(31110): 20只股票 → 检测190对 → 原始通过12对 → 配额6 → 最终选取6对
[协整分析] 周期零售(10280): 10只股票 → 检测45对 → 原始通过2对 → 配额1 → 最终选取1对
[贝叶斯建模] 输入14对 → 成功11对 → 失败3对  ← 暴露黑箱：半导体3对建模失败
[质量筛选] 输入11对 → 通过7对 → 淘汰4对  ← 暴露黑箱：4对质量不达标或冷却期
[配对流程] 协整通过14对 → 贝叶斯成功11对 → 质量筛选7对 → 最终创建7对  ← 完整审计
[配对创建] 各行业配对数量: {'软件': 2, 'REITs': 1, '油气': 1, '周期零售': 1, '电信服务': 1, '公用事业': 1}
```
**优势**：
- ✅ 协整日志不再重复，一行显示完整流程
- ✅ 贝叶斯建模失败暴露（半导体3对失败）
- ✅ 质量筛选淘汰可见（4对被剔除）
- ✅ 配对流程审计一行总览（14→11→7→7）
...
**说明**：
1. 半导体行业配额增大到3对（相比其他行业的1对）
2. 如果检测到6对通过协整，我们选取TOP 3对（按pvalue排序）
3. 这3对会进入后续流程：
   - ✅ 贝叶斯 MCMC 建模（500 warmup + 500 samples, 2 chains）
   - ✅ 质量分数筛选（half_life 60% + mean_reversion_certainty 40%）
   - ✅ 风险配对过滤（DRAWDOWN/ANOMALY冷却期检查）
4. 体现策略根据行业重要性动态调整配额

### 技术要点

#### 风险配对过滤（非黑名单）
**重要澄清**：v7.12.0已移除BlacklistManager模块

**当前机制** (PairSelector.py:163-217):
- `_filter_risk_pairs()` 检查历史配对的 `last_close_reason`
- DRAWDOWN → 180天冷却期（需要重新观察）
- ANOMALY → 永久剔除（订单异常）
- **不依赖trade_count统计**，直接基于平仓原因+冷却期

**设计哲学**：
- ❌ 移除：BlacklistManager（基于trade_count统计的复杂系统）
- ✅ 替换：基于平仓原因的冻结机制（用户所说的"纯粹依赖冻结"）

### 修改文件
- `src/analysis/CointegrationAnalyzer.py:70-71,166-171` - 合并重复日志
- `src/analysis/BayesianModeler.py:60-70` - 添加建模统计
- `src/analysis/PairSelector.py:59-70` - 添加质量筛选统计
- `main.py:200-212` - 添加配对流程审计


---


## [v7.28.2_log-optimization-and-pnl-diagnostics@20250115]

### 版本概述
**日志优化** - 消除重复打印、增强资金可见性、添加 PnL 诊断明细。

### 核心改进

#### 1. 消除 PairDrawdownRule 重复打印
**问题**：
- PairDrawdownRule 触发时打印两次相同日志
- 第一次在 `check()` 方法内部
- 第二次在 RiskManager 中统一打印

**解决**：
- 移除 `PairDrawdownRule.check()` 方法中的 `Debug` 调用
- 保持单一职责：规则只负责检测，日志由 RiskManager 统一管理

**文件**: `src/risk/PairDrawdown.py:155`

#### 2. 选股触发时增加资金状态日志
**改进**：
- 在月度选股触发分隔线后，增加可投资金和已占用资金信息
- 方便评估当前投资容量和资金使用率

**日志格式**：
```
=============================触发第12次选股 (2024-09-03)=============================
[资金状态] 可投资金: $45,231.50 | 总价值: $98,456.23 | 已占用资金: $53,224.73
```

**文件**: `src/UniverseSelection.py:127-135`

#### 3. 平仓日志增加 PnL 明细诊断
**新增功能**：
- 在每次平仓后，额外打印价格变化和各腿 PnL 明细
- Level 1 日志，仅在调试模式下显示

**日志格式**：
```
[PnL明细] ('HST', 'KIM') | 开仓价=(18.3400, 21.5600) | 平仓价=(19.2100, 22.1300) | 数量=(-500, +482) | leg1_pnl=$-435.00 | leg2_pnl=$+274.66
```

**用途**：
- 诊断 Z-score 与 PnL 的非直观关系
- 验证 beta 对冲是否正确执行
- 追踪价格变化对收益的贡献

**文件**: `src/Pairs.py:311-324`

### 文件修改清单
| 文件 | 修改内容 | 行数变化 |
|------|---------|---------|
| `src/risk/PairDrawdown.py` | 移除重复 Debug 调用 | -1 line |
| `src/UniverseSelection.py` | 增加资金状态日志 | +9 lines |
| `src/Pairs.py` | 增加 PnL 明细日志 | +14 lines |

### 预期效果

**修改1 - 消除重复**：
```diff
- [Pair风控] PairDrawdownRule 触发! 配对回撤-单次: 8.5% >= 8.0% ...
- [Pair风控] PairDrawdownRule 触发: 配对回撤-单次: 8.5% >= 8.0% ...  ← 重复
+ [Pair风控] PairDrawdownRule 触发: 配对回撤-单次: 8.5% >= 8.0% ...  ← 只打印一次
```
